...
<div class="admin-content">
    ...
    <form method="post" action="<?= base_url('admin/regimes/store') ?>">
        ...
        <input type="number" step="0.01" id="prix_journalier" name="prix_journalier" value="<?= esc(old('prix_journalier')) ?>" required>

        <label for="variation_poids_hebdo">Variation de poids hebdo</label>
        <input type="number" step="0.01" id="variation_poids_hebdo" name="variation_poids_hebdo" value="<?= esc(old('variation_poids_hebdo')) ?>" required>

        <label for="poids_min_requis">Poids minimum requis</label>
        <input type="number" step="0.01" id="poids_min_requis" name="poids_min_requis" value="<?= esc(old('poids_min_requis', 0)) ?>">

        <label for="pourcentage_viande">Composition viande (%)</label>
        <input type="number" min="0" max="100" id="pourcentage_viande" name="pourcentage_viande" value="<?= esc(old('pourcentage_viande', 0)) ?>">

        <label for="pourcentage_poisson">Composition poisson (%)</label>
        <input type="number" min="0" max="100" id="pourcentage_poisson" name="pourcentage_poisson" value="<?= esc(old('pourcentage_poisson', 0)) ?>">

        <label for="pourcentage_volaille">Composition volaille (%)</label>
        <input type="number" min="0" max="100" id="pourcentage_volaille" name="pourcentage_volaille" value="<?= esc(old('pourcentage_volaille', 0)) ?>">
